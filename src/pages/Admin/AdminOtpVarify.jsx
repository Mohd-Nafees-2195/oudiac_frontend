...
const AdminOtpVarify = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    //   const response = await ApiWithoutAuth.post("/auth/verify-email-admin-otp?email=" + email+"&otp="+otpCode);

    //   if (response.status === 200) {
    //      const token = getJwtFromCookie();
    //      console.log(token);
    //      if(token){
    //         toast("Verification successful!!! nafees");
