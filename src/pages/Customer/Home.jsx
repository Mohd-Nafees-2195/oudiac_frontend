...
function ProductCard({ product }) {
  ...
  const description = product.description || "A captivating blend of rare woods, warm amber, and delicate florals, creating an unforgettable signature scent.";
  ...
  return (
    <div className="group flex flex-col bg-white rounded-xl overflow-hidden hover:shadow-2xl transition-all duration-500 border border-neutral-100 h-full">
      ...
      {/* --- Image Section --- */}
      <Link to={`/product/${product.id}/${product.category.id}`} state={{product}} className="block relative aspect-[4/5] bg-neutral-50 overflow-hidden shrink-0">
        <img
      ...
      <div className="p-5 flex flex-col flex-grow">
        ...
        <p className="text-[10px] font-semibold text-neutral-400 uppercase tracking-[0.2em] mb-1.5">
          {product.category.name || "Fragrance"} • {product.productVariants.length > 0 && product.productVariants?.[0]?.variantType || "100ml"}
        </p>

        {/* Product Name */}
        <Link to={`/product/${product.id}/${product.category.id}`} state={{product}}>
          ...
          <div className="flex items-center gap-0.5 text-amber-500">
            ...
            {[...Array(5)].map((_, i) => (
              <Star
                key={i}
                size={14}
                className={i < Math.floor(rating) ? "fill-amber-500" : "fill-neutral-200 text-neutral-200"}
              />
            ))}
          </div>
          <span className="text-xs font-medium text-neutral-700 ml-1">{rating}</span>
          ...
          <button
            onClick={(e) => {
              ...
              addToCart(product,product.productVariants?.[0],1);
               toast.success(`${product.name} (${product.productVariants?.[0]?.variantType}) added!`);
            }}
...
const Home = () => {
  ...
  const filtered = categories.filter((p) => {
    const matchesCat = activeCategory === 'ALL' || p.category.code === activeCategory;
    return matchesCat;
  });
          ...
          <div className="p-8 space-y-8">
            ...
            <div className="w-full bg-[#53B175] rounded-3xl p-8 flex items-center justify-between overflow-hidden relative">
              ...
              <div className="relative z-10">
                ...
                <h2 className="text-white text-2xl font-bold mb-3">Get the Fragrance at an Affordable Price. ✅</h2>
                <Link
                  to="/browse"
                ...
                {exclusiveOffers.map((offer) => (
                  ...
                  >
                    ...
                    <div className="w-2/5 h-full relative overflow-hidden">
                      ...
                      <img
                        src={offer.image || "https://images.unsplash.com/photo-1594035910387-fea47794261f?q=80&w=800"}
                        alt={offer.name}
