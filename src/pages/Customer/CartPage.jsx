...
export default function CartPage() {
  ...
  const fetchAddress = async () => {
    try {
      const response = await CustomerApi.get("/users/oudiac/get-address", {
        withCredentials: true,
      });
      ...
      toast.error(
        "No Shipping Address Found,Please Add Shipping Address First",
      );
    }
  };
  ...
  const handlePayment = (data) => {
    ...
    const options = {
      ...
      handler: function (response) {
        ...
        navigate("/payment-status", {
          state: { status: "success", data: response.data },
        });
      },
      prefill: {
        name: "Mohd Nafees",
    ...
    rzp1.on("payment.failed", function (response) {
      ...
      navigate("/payment-status", {
        state: { status: "failed", data: response.error },
      });
  ...
  //   // console.log("Polling for order status again...");
  ...
  //         `/orders/oudiac/get/${orderNumber}`,
  //       );
  //       // console.log("Order Status:", response.data);
  //       if (response.data.orderStatus === "CONFIRMED") {
  //         console.log(
  //           "Polling for order status with orderNumber:",
  //           orderNumber,
  //         );
  //         navigate("/payment-status", {
  //           state: "success",
  ...
  //         navigate("/payment-status", { state: "failed", data: response.data });
  //       }
  ...
  //   const intervalId = setInterval(checkOrderStatus, 3000); // Poll every 3 seconds
          ...
          <div className="grid grid-cols-3 gap-8">
            ...
            <div className="col-span-2 space-y-3">
              {cart.map(({ product, variant, quantity }) => (
                <div
                  key={`${product.id}-${variant.variantType}`}
                ...
                >
                  ...
                  <div className="flex items-center gap-3">
                    <button
                      ...
                      onClick={() =>
                        updateQty(product.id, variant.variantType, quantity - 1)
                      }
                    ...
                    <span className="w-6 text-center font-semibold text-[#181725]">
                      {quantity}
                    </span>
                    <button
                      // Pass the variantType to updateQty
                  ...
                  <button
                    ...
                    onClick={() =>
                      removeFromCart(product.id, variant.variantType)
                    }
            ...
            <div className="space-y-4">
              <div className="bg-white rounded-2xl p-6">
                ...
                <div className="space-y-3 mb-4 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium text-[#181725]">
                      ₹{cartTotal.toFixed(2)}
                    </span>
                  </div>
                  {discount > 0 && (
              ...
              <div className="bg-white rounded-2xl p-5">
                <h4 className="font-semibold text-[#181725] mb-3 text-sm">
                  Delivery Details
                </h4>
