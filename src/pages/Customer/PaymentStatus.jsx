...
const PaymentStatus = () => {
  ...
  // Extract the state, providing fallback defaults in case
  // someone navigates here directly without state
  const { status = "success", data = {} } = location.state || {};
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4 font-sans">
      <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-8 text-center border border-slate-100 transition-all duration-300">
        ...
        <h1
          className={`text-2xl font-bold tracking-tight mb-2 ${isSuccess ? "text-slate-900" : "text-rose-600"}`}
        >
        ...
        <div className="bg-slate-50 rounded-xl p-4 mb-8 text-left border border-slate-100 space-y-3">
          <div className="flex justify-between items-center text-xs">
            ...
            <span className="font-mono font-semibold text-slate-700">
              {data.orderNumber || "OUD-2026-XXXX"}
            </span>
          </div>
